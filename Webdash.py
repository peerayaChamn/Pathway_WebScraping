...
def build_graph1(first1, second2):
    ...
    fig = px.pie(dff, values='Total', names='Job_type', hole=.3)
    return fig
...
def build_graph2(first2):
    ...
    fig = px.bar(df[mask], x="Total", y="Job_type", barmode="group", height=600)
    return fig
...
def build_graph3(country1, job1, value):
    ...
    if(job1=='Public Health'):
        skills = ['Communication','Teamwork','Initiative','Interpersonal','Analytics','Flexibility','health']
    elif(job1=='Database'):
        skills = ['Problem-solving','Communication','Oracle','SQL','PowerShell','Security','Analytics','database']
    skills.append(value)
    ...
    fig = px.bar(new, x="Name", y="Code", barmode="group")
    return fig
...
def build_graph3(job1):
    if(job1 == 'Public Health'):
        text = "Communication,Teamwork,Initiative,Interpersonal,Analytics,Flexibility,health"
    if(job1=='Database'):
